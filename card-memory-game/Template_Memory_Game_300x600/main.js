...
function createCard({ scene, x, y, frontTexture, cardName, width, height }) {
  ...
  const flip = (cb) => {
    ...
    isFlipping = true;
    const flipVolume = (scene.config.assets.sounds && scene.config.assets.sounds.flip && scene.config.assets.sounds.flip.volume) || 0.7;
    scene.sound.play('card-flip', { volume: flipVolume });
    scene.tweens.add({
      targets: rotation,
    ...
    this.input.on('pointerdown', (pointer) => {
      ...
      if (this.cardOpened) {
        if (this.cardOpened.cardName === card.cardName) {
          card.flip(() => {
            const matchVolume = (this.config.assets.sounds && this.config.assets.sounds.match && this.config.assets.sounds.match.volume) || 0.7;
            this.sound.play('card-match', {volume: matchVolume});
          ...
          card.flip(() => {
            const mismatchVolume = (this.config.assets.sounds && this.config.assets.sounds.mismatch && this.config.assets.sounds.mismatch.volume) || 0.7;
            this.sound.play('card-mismatch', {volume: mismatchVolume});
