...
class MainScene extends Phaser.Scene {
  constructor() {
    super("MainScene");
    this.configData = window.GAME_CONFIG;
    this.score = 0;
    this.timer = 30;
  ...
  create() {
    ...
    this.scoreText = this.add.text(cfg.ui.scoreText.x, cfg.ui.scoreText.y, cfg.ui.scoreText.label + this.score, {
      fontSize: cfg.ui.font, fill: cfg.ui.color
    }).setDepth(3);
    ...
    // Colisiones
    this.physics.add.overlap(this.player, this.ingredients, this.catchIngredient, null, this);
  }
  ...
  endGame() {
    ...
    this.add.text(150, 300 + cfg.ui.endTitle.offsetY, cfg.ui.endTitle.text, {
      fontSize: cfg.ui.endTitle.fontSize,
      fill: cfg.ui.color
    }).setOrigin(0.5).setDepth(5);

    this.add.text(150, 300 + cfg.ui.endText.offsetY + 30, cfg.ui.endText.text + this.score, {
      fontSize: cfg.ui.endText.fontSize,
      fill: cfg.ui.color
    }).setOrigin(0.5).setDepth(5);

    const btn = this.add.text(150, 360, cfg.ui.buttonText.text, {
      fontSize: cfg.ui.buttonText.fontSize,
      fill: cfg.ui.color
    })
    ...
    btn.on("pointerdown", () => {
      window.open(cfg.ui.learnMoreUrl, "_blank");
    });
  }
}